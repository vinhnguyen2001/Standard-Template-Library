...
Snode* findMaxValue(Snode* curr, Snode*& max);
...
int sumEvenNumbers(Snode* cur, int sum) {
	...
	/*
	tinh tong cua phan tu chan trong mang.
...
Snode* findMaxValue(Snode*curr, Snode*& max ) {
	/*
	Tim gia tri lon nhat trong danh sach lien ket & tra ve con tro giu dia chi cua bien do.
	...
	*/
	if (curr->pnext == nullptr) {
		...
		Snode* nextElement = curr->pnext;

		if (max->data < nextElement->data) {
			max = nextElement;
		}
		return findMaxValue(nextElement, max);
	}
}
